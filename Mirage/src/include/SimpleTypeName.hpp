...
template <typename T>
std::string type_string_raw_vptr(T* ptr) {
    assert(ptr != nullptr && "ptr was null");

    static_assert(std::is_polymorphic_v<T>, "T should be a polymorphic type you should proc this func to get type benhind a ptr");
...
inline std::string remove_prefix_withoutPrefix(std::string&& s) {
    ...
    if (s.find(class_prefix) == 0)
        return s.substr(class_prefix.length());
    else if (s.find(struct_prefix) == 0)
        return s.substr(struct_prefix.length());
    return s;
}
...
template <typename T>
inline std::string get_type_name(T* ptr) {
    return skydown::detail::remove_prefix_withoutPrefix(skydown::detail::type_string_raw_vptr(ptr));
}
